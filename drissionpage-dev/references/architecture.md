...
> 本文件只在维护 DrissionPage 上游仓库、排查源码级问题或同步文档时使用。
> 普通项目里的脚本编写、页面调试和示例改写，不要把本文件当默认入口；默认先看 `references/docs-map.md` 和 `references/docs/`。
...
- 当前安装包公开导出确实只包含：`Chromium`、`ChromiumPage`、`SessionPage`、`WebPage`、`ChromiumOptions`、`SessionOptions`、`__version__`。
- 当前安装包版本号来自 `version.py`。
- `WebPage` 真实继承顺序为 `SessionPage, ChromiumPage, BasePage`，默认 `mode='d'`。
- `ChromiumPage` 在 `__new__()` 中按 `browser.id` 复用实例，`_PAGES` 是浏览器级单例缓存。
- `dp` CLI 入口在 `_functions/cli.py`，已确认支持：
  - `--set-browser-path`
  - `--set-user-path`
  - `--configs-to-here`
  - `--launch-browser`
- `configs_to_here()` 在 `_functions/tools.py` 中默认输出 `dp_configs.ini`。
