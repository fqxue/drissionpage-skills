...
> 来源：https://www.drissionpage.cn/browser_control/actions
...
| 参数名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `ele_or_loc` | `str`<br>`ChromiumElement` | 必填 | 接收拖动动作的元素定位符 |
| `files` | `str`<br>`list`<br>`tuple` | `None` | 要拖入文件路径，可多个，不为`None`时下面参数无效 |
