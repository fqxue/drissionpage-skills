# 骚神视频实战教学代码（DrissionPage 实战教学源码）

> 来源：https://wxhzhwxhzh.github.io/sao/teach_code/实战代码.html
>
> 本文件是该页面的完整代码存档，作为 DrissionPage skill 的唯一 API 参考文档。所有写法均以本文件中的示例为准。
...
js='''const ws = new WebSocket('wss://echo.websocket.org'); // 使用公共回声服务器进行测试
...
# --- 配置区 ---
TARGET_URL = 'https://www.bigee.cc/book/20233/'
MAX_WORKERS = 4  # 题目要求：最大线程 4
...
def get_task_list():
    ...
    # 提取所有章节链接
    links = [(i, el.link) for i, el in enumerate(main_tab('.listmain').eles('t:a')) if 'book' in el.link]
    logger.info(f"成功解析目录，共 {len(links)} 章节")
    return links

def download_chapter(task):
    """单个章节的下载逻辑"""
    index, url = task
    # 为每个线程创建一个新标签页
    tab = browser.new_tab(url)
    try:
        # 获取内容（添加简单等待确保加载）
        title = tab.ele('#title').text
        content = tab.ele('#chaptercontent').text

        filename = f"{index:04d}_{title}.txt"
        # 简单清洗文件名，防止特殊字符报错
        valid_filename = "".join([c for c in filename if c.isalnum() or c in "._- "]).strip()

        with open(valid_filename, 'w', encoding='utf-8') as f:
            f.write(f"{title}\n\n{content}")

        logger.success(f"已保存: {valid_filename}")
    except Exception as e:
...
def main():
    ...
    if not tasks:
        logger.warning("任务列表为空，请检查 URL 或网页结构。")
        return
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        executor.map(download_chapter, tasks)

    # 3. 清理
    browser.quit()
    logger.info("全部任务处理完毕。")
...
DB_FILENAME = "xiaohongshu.db"
MAX_COUNT = 12


def extract_note_data(data: dict) -> dict | None:
    """从抓包响应中提取笔记核心字段"""
    if not isinstance(data, dict):
        return None

    note = (
        data.get('data', {}).get('items', [{}])[0].get('note_card', {})
        or data.get('note', {})
    )
    if not note:
        return None
...
def keyword_to_table_name(keyword: str) -> str:
    """将关键词转为合法的 SQLite 表名"""
    clean = re.sub(r'[^\w\u4e00-\u9fff]', '_', keyword.strip())
    return f"notes_{clean}"


def save_to_db(db: sqlite_utils.Database, table_name: str, info: dict):
    """写入数据库，表不存在时自动创建"""
    db[table_name].insert(info)


def scroll_and_wait(tab, distance: int = 1200, wait_range=(1, 2)):
    logger.info('📜 滚动加载更多内容...')
    tab.scroll.down(distance)
    tab.wait(*wait_range)


def click_card(tab, card) -> bool:
    """点击卡片，优先点击图片元素，返回是否成功"""
    try:
        target = card.ele('xpath:./div/a[2]/img') or card
        target.click(by_js=True)
        return True
    except Exception as e:
        logger.error(f'点击卡片失败: {e}')
        return False


def close_detail(tab):
    """关闭详情弹窗"""
    tab.actions.type(Keys.ESCAPE)
    tab.wait(1.5,2.5)


def crawl_keyword(tab, db: sqlite_utils.Database, keyword: str):
    """爬取单个关键词的数据"""
    table_name = keyword_to_table_name(keyword)
    seen_indices = set()
    data_count = 0

    logger.info(f'🚀 开始爬取关键词: 【{keyword}】→ 表: {table_name}')

    tab.listen.start('web/v1/feed')
    tab.get(f'https://www.xiaohongshu.com/search_result?keyword={keyword}&source=web_profile_page')
    tab.wait.load_start()

    while data_count < MAX_COUNT:
        cards = tab.eles('xpath://*[@id="global"]/div[2]/div[2]/div/div/div[3]/div[1]/section')

        if not cards:
            scroll_and_wait(tab, distance=1000, wait_range=(1, 1))
            continue
        ...
        for card in cards:
            ...
            logger.info(f'  → 爬取第 {data_count + 1} 条...')

            if not click_card(tab, card):
                continue

            res = tab.listen.wait(timeout=4)
            if res:
                info = extract_note_data(res.response.body)
                if info:
                    save_to_db(db, table_name, info)
                    logger.info(f'  💾 已存储: {info["标题"] or "(无标题)"}')
                    data_count += 1
                else:
                    logger.warning('  ⚠️ 数据解析为空，跳过')
            else:
                logger.warning(f'  ⏱️ 第 {data_count + 1} 条抓包超时')
    ...
    tab.listen.stop()
    logger.success(f'✅ 【{keyword}】采集完成，共 {data_count} 条 → 表: {table_name}')
