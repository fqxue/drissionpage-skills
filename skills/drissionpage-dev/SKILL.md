...
1. **实战示例优先**：先查 `references/docs/实战示例/`，复用已验证的写法和调用链。**凡是编写 DrissionPage 相关代码，建议先阅读实战示例并参考其代码风格。**
...
> **核心原则：凡是编写 DrissionPage 相关代码，建议优先参考 `references/docs/实战示例/` 中的教学示例，并尽量保持与示例一致的代码风格。可优先复用示例中已验证的模式。**

以下规则从 demo 示例中提炼，生成 DrissionPage 代码时可优先参考：
...
1. 接收并确认用户需求（目标站点、预期结果、输入输出约束）。
2. 先复述并拆解需求，明确采集目标、关键页面与成功标准。
3. 先用 `chrome-devtools-mcp`（或同类 MCP）调试网页，梳理稳定的爬取/交互流程。
4. 在流程明确后，再准备使用 DrissionPage 设计最小可行实现（对象选型、步骤顺序、等待策略）。
5. 编写代码前按优先级查文档：`references/docs/实战示例/` → `references/docs/入门指南/` → `references/docs/` 其余栏目。
6. 编写与调试代码时优先回查文档和示例，非必要不读源码；仅当文档与行为明显冲突时再核对源码。
7. 完成任务并做最小验证，确保结果与最初需求一致。

## 仓库内规则

- 编写任何 DrissionPage 代码前，建议先阅读 `references/docs/实战示例/` 中的示例，并参考上方“代码风格规范”中的风格要求。
...
- 需要调试浏览器或分析网站时，优先使用 `chrome-devtools-mcp`（或用户已提供的同类 MCP）；仅在 MCP 工具不可用时，才回退到 DrissionPage 的 CDP 能力（`references/docs/控制浏览器/🛰️ 页面交互.md` 中 `cdp()`/`run_cdp()` 方法）。
- 默认采用“三段式协作”：① 优先用 MCP 做 DevTools 诊断（Network/Performance/Console）→ ② DrissionPage 复现并最小化自动化步骤 → ③ 回到 DrissionPage 落地修复与回归验证。
